...
- **transactions** are a way to group reads and writes into one unit of work that either succeeds (*commits*) or fails (*abort*, or *rollback*) to simplify the programming model for applications accessing a database
...
- **Atomicity**
  ...
  > not a principle of concurrency -- in some domains, it might mean that no threads could see a half-finished state
- **Consistency**
  - *consistency* is an overloaded term -- might mean: replica consistency, consistent hashing, linearizability in the CAP theorem, or in the ACID context being in a good state
